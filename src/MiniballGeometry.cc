...
void MiniballGeometry::SetupCluster() {
	...
	for( unsigned char i = 0; i < ncry; i++ ) {
		...
		for( unsigned char j = 0; j < nseg; j++ ) {
			...
			//seg_offset[i][j].RotateY( TMath::ATan( ( DIST_CORE_CORNER + 0.5*INTER_CRYSTAL_GAP ) / NOMINAL_CLUSTER_DIST ) ); // new
			seg_offset[i][j].RotateY( TMath::ATan( DIST_CORE_CORNER / NOMINAL_CLUSTER_DIST ) ); // old
			...
			//seg_offset[i][j].SetZ( seg_offset[i][j].Z() - NOMINAL_CLUSTER_DIST + INTERACTION_DEPTH + r ); // new
			seg_offset[i][j].SetZ( seg_offset[i][j].Z() - NOMINAL_CLUSTER_DIST + r ); // old
